...
public interface DirpernatRepository extends JpaRepository<Dirpernat, Long> {
    ...
    @Query("select dirpernat from Dirpernat dirpernat right join Trabajador trabajador on dirpernat.pernatural.id=trabajador.pernatural.id  where trabajador.id=?1 and dirpernat.nFlgactivo = true")
    // @Query("select new map((select dep.vDesdep from Departamento dep where dep.vCoddep=dirpernat.nCoddepto) as dpto, (select provin.vDespro from Provincia provin where provin.vCodpro=dirpernat.nCodprov) as prov, (select distri.vDesdis from Distrito distri where provin.vCoddis=dirpernat.nCoddist) as dist, dirpernat as direc) from Dirpernat dirpernat inner join Trabajador trabajador on dirpernat.pernatural.id=trabajador.pernatural.id  where trabajador.id=?1 and dirpernat.nFlgactivo = true")
